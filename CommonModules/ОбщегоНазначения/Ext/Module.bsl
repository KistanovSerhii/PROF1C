...
Функция ПолучитьСлучайныйНаборБилетов(ИмяСправочникаЭкзамен, РазделКод = 0, КоличествоБилетов = 0) Экспорт
	...
	Пока КолБилетов > ИтераторБилетов Цикл
		...
		БилетСсылка			= ПолучитьСлучайныйКодБилетаУникальныйВГруппе(СправочникГруппа, мТекущийНабор, ИмяСправочникаЭкзамен);
		...
		Если БилетСсылка <> Неопределено И БилетСсылка <> Справочники[ИмяСправочникаЭкзамен].ПустаяСсылка() Тогда
			...
			ОписаниеБилета = Новый Структура;
			
			ОписаниеБилета.Вставить("Родитель", СправочникГруппа);
			ОписаниеБилета.Вставить("Ссылка", 	БилетСсылка);
			
			мНаборБилетов.Добавить(ОписаниеБилета);
			мТекущийНабор.Добавить(БилетСсылка);
		КонецЕсли;
